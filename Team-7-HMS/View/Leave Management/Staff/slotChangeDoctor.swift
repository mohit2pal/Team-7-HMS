...
struct slotChangeDoctor: View {
    ...
    @State private var slotDetails : [String : Any]? // Optional dictionary
    ...
    var body: some View {
        ScrollView{
            ...
            .onChange(of: changeDate, { oldValue, newValue in
                ...
                fetchDoctorsDetails(name: doctor.name) { details, error in
                    ...
                    FirebaseHelperFunctions().fetchSlots(doctorID: doctorUid, date: formattedDate) { slots, error in
                        if let error = error {
                            self.slotDetails = nil // Reset slotDetails to nil if there's an error
                            ...
                            if let slots = slots {
                                ...
                                self.slotDetails = nil // Reset slotDetails to nil if there are no slots available
